...
public interface IToolProviderBlock {
    ...
    default int getToolLevel(Level world, BlockPos pos, BlockState blockState, ItemAbility ItemAbility) {
        return -1;
    }
    ...
    default Map<ItemAbility, Integer> getToolLevels(Level world, BlockPos pos, BlockState blockState) {
        return getTools(world, pos, blockState).stream()
                .collect(Collectors.toMap(Function.identity(), ItemAbility -> getToolLevel(world, pos, blockState, ItemAbility)));
    }
