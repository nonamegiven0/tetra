...
public class ConfigHandler {
	...
	public static TetraConfig config;
	...
	static {
		...
		enableSingle = builder.comment("Enable modular single headed implements").worldRestart().define("single_headed",
				true);
		...
		experimentalFeatures = builder
				.comment("Features that are considered experimental can be listed here to enable them")
				.defineListAllowEmpty("experimental_features", Collections.emptyList(),
						o -> FeatureFlag.matchesAnyKey(o));
		...
		magicCapacityMultiplier = builder.comment(
		...
		settleLimitBase = builder.comment("The base value for number of uses required for a module to settle")
				.defineInRange("settle_base", 270, Integer.MIN_VALUE, Integer.MAX_VALUE);

		settleLimitLevelMultiplier = builder.comment(
		...
		settleLimitDurabilityMultiplier = builder.comment(
		...
		honeSwordBase = builder.comment("The base value for number of uses required before a sword can be honed")
				.defineInRange("hone_sword_base", 110, Integer.MIN_VALUE, Integer.MAX_VALUE);

		honeSwordIntegrityMultiplier = builder.comment(
		...
		honedoubleBase = builder.comment("The base value for number of uses required before a tool can be honed")
				.defineInRange("hone_double_base", 140, Integer.MIN_VALUE, Integer.MAX_VALUE);

		honedoubleIntegrityMultiplier = builder.comment(
		...
		honeBowBase = builder.comment("The base value for number of uses required before a bow can be honed")
				.defineInRange("hone_bow_base", 48, Integer.MIN_VALUE, Integer.MAX_VALUE);

		honeBowIntegrityMultiplier = builder.comment(
		...
		honeShieldBase = builder.comment("The base value for number of uses required before a shield can be honed")
				.defineInRange("hone_shield_base", 48, Integer.MIN_VALUE, Integer.MAX_VALUE);

		honeShieldIntegrityMultiplier = builder.comment(
		...
		honeCrossbowBase = builder.comment("The base value for number of uses required before a crossbow can be honed")
				.defineInRange("hone_crossbow_base", 48, Integer.MIN_VALUE, Integer.MAX_VALUE);

		honeCrossbowIntegrityMultiplier = builder.comment(
		...
		honeSingleIntegrityMultiplier = builder.comment(
		...
		enableExtractor = builder.comment("Enable the extractor bedrock functionality").worldRestart()
...
//        spec = builder.build();
		...
		Pair<TetraConfig, ModConfigSpec> confPair = builder.configure(TetraConfig::new);
		spec = confPair.getRight();
		config = confPair.getLeft();
	}

	public static void setup(ModContainer container) {
		// this is slightly more complicated than just calling
		// ModLoadingContext.registerConfig but it allows us to preserve insertion order
		// which makes the config easier to read
		final CommentedFileConfig configData = CommentedFileConfig
				.builder(FMLPaths.CONFIGDIR.get().resolve("tetra.toml")).sync().autosave().preserveInsertionOrder()
